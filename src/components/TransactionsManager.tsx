...
import { MoreHorizontal, Download } from 'lucide-react'; // Import the Download icon
          ...
          {dataType === 'grouped' ? (
            <div className='space-y-6'>
              {(transactionData as TransactionGroup[]).map((group) => {
                ...
                // Get the current source filter from the URL to build the export link
                const currentSource = searchParams.get('source') || 'All';
                // Extract the YYYY-MM period for the specific month of this group
                  ...
                  <div key={group.period}>
                    <div className='flex justify-between items-center bg-muted/50 p-3 rounded-t-lg border'>
                      ...
                      <div className='flex items-center text-xs space-x-4'>
                        ...
                        {/* Only show the download button for monthly groups */}
                        {activeGroupBy === 'month' && (
                          <a
                            href={exportUrl}
